...
namespace TeamMatching.Web.Controllers
{
    ...
    public class MyActivitiesController : ControllerBase
    {
        ...
        [Authorize]
        [HttpGet("review")]
        public async Task<ActionResult<GetReviewTargetResponse>> GetReviewTarget([FromBody] GetReviewTargetRequest request)
        {
            // 모델 유효성 검사 (DTO의 [Required] 등 체크)
            if (!ModelState.IsValid)
            {
            ...
            var result = await _myActivitiesService.GetReviewTargetAsync(request, userId);
        ...
        public async Task<ActionResult<SubmitReviewResponse>> SubmitReview([FromBody] SubmitReviewRequest request)
        {
            ...
            var result = await _myActivitiesService.SubmitReviewAsync(request, userId);
